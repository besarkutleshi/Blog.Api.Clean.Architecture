...
public class UpdatePostTests
{
    ...
    {
        var dto = new UpdatePostDto(1, "Valid Contect", "Valid Title", "Valid FriendlyUrl");

        var postRepositoryMock = new Mock<IPostRepository>();
        postRepositoryMock.Setup(x => x.UpdatePost(It.IsAny<Post>(), It.IsAny<CancellationToken>()))
        ...
        var handler = new UpdatePostCommandHandler(postRepositoryMock.Object, mapperMock.Object);
    ...
    [Fact]
    public async Task Should_Have_No_Error_When_Repository_And_Update_Model_Are_Valid()
    {
        var post = new Post();
        var dto = new UpdatePostDto(1, "Valid Contect", "Valid Title", "Valid FriendlyUrl");
        ...
        var postRepositoryMock = new Mock<IPostRepository>();
        postRepositoryMock.Setup(x => x.UpdatePost(It.IsAny<Post>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Success(Success.Ok(post)));
        ...
        Assert.Equal(SuccessType.Success, result.Response.Type);
